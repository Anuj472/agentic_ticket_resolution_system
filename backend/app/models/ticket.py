...
from sqlalchemy import String, Text, DateTime, Enum as SAEnum, ForeignKey, Float, JSON, Boolean
...
class Ticket(Base):
    ...
    priority: Mapped[TicketPriority] = mapped_column(SAEnum(TicketPriority), default=TicketPriority.MEDIUM, index=True)
    channel: Mapped[TicketChannel]   = mapped_column(SAEnum(TicketChannel), default=TicketChannel.WEB)
    ...
    group_id: Mapped[uuid.UUID]     = mapped_column(UUID(as_uuid=True), ForeignKey("agent_groups.id"), nullable=True)
    ...
    parent_ticket_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("tickets.id"), nullable=True)
