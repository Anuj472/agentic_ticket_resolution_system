...
async def import_csv():
    ...
    async with async_session() as db:
        ...
        # Check existing ticket numbers to avoid unique constraint violations
        existing_nums_res = await db.execute(select(Ticket.ticket_number))
        existing_nums = set(existing_nums_res.scalars().all())

        rows = []
        with open("/app/tickets_import.csv", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                tnum = gen_ticket_number(row["ticket_id"])
                ...
                ticket = Ticket(
                    ...
                    category_confidence=float(row["confidence_score"])
                    ...
                    is_escalated=row.get("escalation_required", "false").strip().lower()
                    == "true",
