...
{{
  ...
  "keywords": ["kw1","kw2","kw3"]
    ...
    try:
        ...
        logger.info(f"[LLM] Classified → {result.get('category')} / {result.get('priority')}")
        return result
    except Exception as e:
        logger.error(f"[LLM] classify_ticket failed: {e}")
        return {
            "category": "Application", "sub_category": "General Issue",
            "priority": "medium", "sentiment": "neutral",
            "urgency_score": 0.5, "summary": title[:100], "keywords": [],
        }
...
async def generate_solution(
    ...
    if similar_tickets:
        for i, t in enumerate(similar_tickets[:3], 1):
            resolution = t.get("resolution") or t.get("suggested_solution") or t.get("content", "")
            if resolution:
                past_context += (
...
3. "L1"           — for priority "high" (needs human review)"""
    try:
        ...
        result = json.loads(resp.choices[0].message.content)
        logger.info(f"[LLM] Routing → {result.get('route_to')} (conf={result.get('confidence', 0):.2f})")
        return result
    except Exception as e:
        logger.error(f"[LLM] routing_decision failed: {e}")
        priority = context.get("priority", "medium").lower()
        if priority == "critical":
            return {"route_to": "escalate",      "confidence": 0.9, "reason": "Critical priority — escalate"}
        if priority in ("low", "medium"):
            return {"route_to": "auto_resolve",  "confidence": 0.8, "reason": "Low/medium — auto-resolve"}
        return     {"route_to": "L1",            "confidence": 0.7, "reason": "High priority — L1 review"}
