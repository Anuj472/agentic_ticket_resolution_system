...
def extract_keywords(text: str) -> list[str]:
    ...
    stop_words = {
        ...
        'not','with','this','that','have','has','had','be','been','was','were',
        'are','will','would','could','should','from','by','as','into','about',
    }
...
def extract_entities(text: str) -> dict:
    ...
    entities["services"]    = re.findall(r'\b[A-Z][a-z]+ (?:Server|Service|Portal|Gateway|Cluster|Host)\b', text)
    # Error codes
    entities["error_codes"] = re.findall(r'\b(?:error|code|err)\s*[:\-]?\s*(\d{3,4})\b', text, re.IGNORECASE)
    # URLs
