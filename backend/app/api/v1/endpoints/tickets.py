...
from app.schemas.ticket import TicketCreate, TicketUpdate, TicketResponse, TicketListResponse, TicketResolve
router = APIRouter()
...
async def create_ticket(
    ...
    ticket = Ticket(
        ...
        priority      = TicketPriority[payload.priority.value.upper()] if hasattr(payload.priority, 'value') else TicketPriority.MEDIUM,
        channel       = TicketChannel[payload.channel.value.upper()] if hasattr(payload.channel, 'value') else TicketChannel.WEB,
...
async def get_unprocessed_queue(
    ...
    q = select(Ticket).where(
        ...
        or_(
            Ticket.ai_suggested_solution.isnot(None),
...
async def resolve_ticket(
    ...
    await db.commit()
    await db.refresh(ticket)
    return {"message": "Ticket resolved", "ticket_number": ticket.ticket_number, "status": "RESOLVED"}
